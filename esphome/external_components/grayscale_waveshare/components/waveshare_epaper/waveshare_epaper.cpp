...
static uint8_t color_to_gray_exp_2bit(Color color) {
  uint16_t luminance = (uint16_t(color.red) * 30u + uint16_t(color.green) * 59u + uint16_t(color.blue) * 11u) / 100u;
  if (color.white > luminance)
    luminance = color.white;

  if (luminance >= 224u)
    return 0x03;  // white
  if (luminance >= 160u)
    return 0x02;  // light gray
  if (luminance >= 64u)
    return 0x01;  // dark gray
  return 0x00;    // black
}

void WaveshareEPaper7P5InV2PGrayExperimental::initialize() {
  // 4-gray init for the 7.5" V2P panel, based on the known-good ESPHome V2P
  // power-up path plus the vendor grayscale enable value.
  if (this->reset_pin_ != nullptr) {
    this->reset_pin_->digital_write(true);
    delay(20);
    this->reset_pin_->digital_write(false);
...
void WaveshareEPaper7P5InV2PGrayExperimental::fill(Color color) {
  if (this->get_clipping().is_set()) {
    Display::fill(color);
    return;
  }

  const uint8_t shade = color_to_gray_exp_2bit(color);
  const uint8_t packed = (shade << 6) | (shade << 4) | (shade << 2) | shade;
  memset(this->buffer_, packed, this->get_buffer_length_());
}

void HOT WaveshareEPaper7P5InV2PGrayExperimental::draw_absolute_pixel_internal(int x, int y, Color color) {
  if (x >= this->get_width_internal() || y >= this->get_height_internal() || x < 0 || y < 0)
    return;

  const uint8_t shade = color_to_gray_exp_2bit(color);
  const uint32_t pos = (x + y * this->get_width_internal()) / 4u;
  const uint8_t shift = 6u - ((x & 0x03u) * 2u);
  this->buffer_[pos] = (this->buffer_[pos] & ~(0x03u << shift)) | (shade << shift);
}

uint32_t WaveshareEPaper7P5InV2PGrayExperimental::get_buffer_length_() {
  return this->get_width_internal() * this->get_height_internal() / 4u;
}

void HOT WaveshareEPaper7P5InV2PGrayExperimental::display() {
  const uint32_t packed_len = this->get_buffer_length_();
  const uint32_t plane_len = packed_len / 2u;

  ESP_LOGI(TAG, "Power on the display and hat for 4-gray refresh");
  ...
  for (uint32_t i = 0; i < plane_len; i++) {
    uint8_t temp3 = 0;
    for (uint8_t j = 0; j < 2; j++) {
      uint8_t temp1 = this->buffer_[i * 2u + j];
      for (uint8_t k = 0; k < 2; k++) {
        uint8_t temp2 = temp1 & 0xC0;
        if (temp2 == 0x00 || temp2 == 0x80)
          temp3 |= 0x01;
        temp3 <<= 1;
...
void WaveshareEPaper7P5InV2PGrayExperimental::dump_config() {
  LOG_DISPLAY("", "Waveshare E-Paper", this);
  ESP_LOGCONFIG(TAG, "  Model: 7.50inv2p-grayexp (4-gray)");
  LOG_PIN("  Reset Pin: ", this->reset_pin_);
  LOG_PIN("  DC Pin: ", this->dc_pin_);
  LOG_PIN("  Busy Pin: ", this->busy_pin_);
  LOG_UPDATE_INTERVAL(this);
